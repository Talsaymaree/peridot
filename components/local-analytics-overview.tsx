              ...
              <div className="space-y-4">
                ...
                <div className="peridot-panel p-6">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                      <div className="peridot-section-label text-xs text-white/45">By Weekday</div>
                      <h3 className="peridot-panel-heading mt-2 text-2xl font-semibold text-white">Weekly rhythm</h3>
                    </div>
                    {!isLoading ? (
                      <div className="rounded-full border border-emerald-200/20 bg-emerald-200/10 px-3 py-1 text-xs uppercase tracking-[0.16em] text-emerald-100/80">
                    ...
                    <div className="mt-5 space-y-3">
                      {weekdayBreakdown.map((item) => (
                        <div key={item.day} className="grid grid-cols-[3rem_1fr_auto] items-center gap-3">
                          <div className="peridot-stat-label text-xs text-white/50">{item.label}</div>
                          <div className="rounded-full bg-white/[0.05] p-[3px]">
                            <div
                              className="h-3 rounded-full bg-gradient-to-r from-emerald-300 via-lime-200 to-cyan-200"
                              style={{ width: `${Math.max((item.total / peakWeekdayValue) * 100, item.total > 0 ? 14 : 0)}%` }}
                            />
                          </div>
                          <div className="text-sm font-medium text-white/72">{item.total}</div>
                        </div>
                      ))}
                    </div>
                  ) : (
